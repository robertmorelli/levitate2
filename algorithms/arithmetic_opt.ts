...
// Tunable confidence threshold. 0.55 requires the target token to capture
// at least 55% of the residual's semantic direction.
const MIN_ALIGNMENT = 0.55;

register("arithmetic_opt", (ctx, tokens) => {
  ...
  tokens.forEach((_, i) => {
    ...
    for (let j = 0; j < MAX_PER_TOKEN; j++) {
      ...
      // The early exit: drop the token if it's a weak geometric echo
      if (alignment < MIN_ALIGNMENT) break;
