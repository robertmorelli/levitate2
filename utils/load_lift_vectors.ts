...
import { build_nearest_token_fn, parse_nearest_metric, type NearestMetric, type NearestTokenFn } from "./find_nearest_token.js";
...
export function load_lift_vectors(): LiftVectors {
  ...
  const nearest_metric = parse_nearest_metric(process.env["NEAREST_METRIC"]);
  const ptx_vectors  = load_vector_map(ptx_path);
  const llvm_vectors = load_vector_map(llvm_path);
  const find_nearest_llvm = build_nearest_token_fn(llvm_vectors, nearest_metric);
  return { ptx_vectors, llvm_vectors, find_nearest_llvm, nearest_metric };
}
